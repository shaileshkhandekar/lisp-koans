...
(defun list-to-bit-vector (my-list)
  "Creates a bit-vector based on the no. of elements in my-list.
   E.g. (list-to-bit-vector '(1 1 0)) => #*110
        (list-to-bit-vector '(0 0 1 1 0 0 1 1)) => #*00110011"
  (let ((result-bits (make-array 
		      (length my-list) 
		      :element-type 'bit))
	(i 0))
    (dolist (e my-list)
      (setf (bit result-bits i) e)
      (incf i))
    result-bits))
